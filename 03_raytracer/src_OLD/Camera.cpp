...
Camera::Camera(const Vector3D& position, const Vector3D& direction, double fov, double aspect_ratio) :
    position(position), direction(direction.normalize()), fov(fov), aspect_ratio(aspect_ratio) {}
...
Ray Camera::get_ray(double u, double v) const {
    ...
    Vector3D lower_left = position + direction - (right * (viewport_width / 2)) - (up * (viewport_height/2));
